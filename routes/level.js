...
router.post('/update-level', (req, res) => {
  ...
  jwt.verify(token, process.env.JWT_SECRET, (err, decoded) => {
    ...
    db.query('UPDATE users SET selected_level = ? WHERE id = ?', [selected_level, user_id], (err, result) => {
      if (err) { console.error(err); return res.status(500).json({ success: false, error: 'Database error' }); }
...
router.get('/upgrade-check', auth, (req, res) => {
  db.query('SELECT current_level FROM students WHERE user_id = ? LIMIT 1', [req.userId], (err, rows) => {
    ...
    db.query(
      ...
      (err2, attempts) => {
        ...
        if (total < 7) {
          return res.json({ success: true, eligible: false, quizzesCompleted: total, quizzesRequired: 7, currentLevel, nextLevel });
        }

        const avg = attempts.reduce((sum, a) => sum + a.score_percentage, 0) / total;
        return res.json({
          success: true, eligible: avg > 70,
          averageScore: Math.round(avg), quizzesCompleted: total, quizzesRequired: 7, currentLevel, nextLevel,
        });
      }
    );
  });
...
router.post('/upgrade', auth, (req, res) => {
  db.query('SELECT current_level FROM students WHERE user_id = ? LIMIT 1', [req.userId], (err, rows) => {
    ...
    db.query('UPDATE students SET current_level = ? WHERE user_id = ?', [nextLevel, req.userId], (err2) => {
      if (err2) return res.status(500).json({ success: false, error: 'Database error' });

      // ── Send level_change notification ──────────────────
      NotificationService.create(
        req.userId,
        'level_change',
        '⚡ Level Upgraded!',
        `Congratulations! You moved from ${currentLevel} to ${nextLevel}`
      ).catch(e => console.warn('[upgrade] notification error:', e.message));
