...
// ============================================================
//  ADD THESE ROUTES to your routes/studentCourse.js
//  Place them BEFORE the final `module.exports = router;`
//
//  These 2 endpoints power the complete dynamic quiz system:
//    GET  /api/student/courses/:courseId/quiz        → fetch questions
//    POST /api/student/courses/:courseId/quiz/submit → submit + score
//
//  DB tables needed (run quiz_schema.sql first):
//    quiz_questions   (already exists — teacher questions)
//    quiz_attempts    (new — stores each attempt)
//    quiz_answers     (new — stores each answer per attempt)
// ============================================================

// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  GET /api/student/courses/:courseId/quiz
//
//  Fetches quiz questions for the student's level.
//  Does NOT send correct_answer_index to the client.
//
//  Query params:
...
//  {
//    "success": true,
//    "quiz_title": "Protein Synthesis Quiz",
//    "course_level_id": 42,
//    "total": 5,
//    "questions": [
//      { "id": 1, "question_text": "...", "options": ["A","B","C","D"] },
//      ...
//    ]
//  }
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
router.get('/:courseId/quiz', auth, (req, res) => {
  ...
  db.query(
    ...
    (err, levelRows) => {
      if (err) {
        console.error('[quiz/get] course_levels error:', err);
        return res.status(500).json({ success: false, message: 'Database error' });
      }
      if (!levelRows.length)
        return res.status(404).json({
          success: false,
          message: 'No content found for your level in this course',
        });

      const courseLevelId = levelRows[0].id;

      // Step 2: fetch questions — do NOT return correct_answer_index
      db.query(
        ...
        (err2, questions) => {
          if (err2) {
            console.error('[quiz/get] quiz_questions error:', err2);
            return res.status(500).json({ success: false, message: 'Database error' });
          }
          ...
          return res.status(200).json({
            success:         true,
            quiz_title:      'Mid-course Quiz',   // or fetch from a quizzes table later
...
//
//  Submits student answers, calculates score, stores attempt.
//
//  Body (JSON):
//  {
//    "level": "Beginner",
...
//    "passed": true,
//    "details": [
//      { "question_id": 1, "selected_index": 2, "correct_index": 2, "is_correct": true },
//      ...
//    ]
//  }
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
router.post('/:courseId/quiz/submit', auth, (req, res) => {
  ...
  db.query(
    ...
    (err, levelRows) => {
      ...
      // ── Step 2: Fetch all correct answers from DB ────────
      ...
      db.query(
        ...
        (err2, correctRows) => {
          ...
          // Build a map: question_id → correct_answer_index
          const correctMap = {};
          correctRows.forEach(r => { correctMap[r.id] = r.correct_answer_index; });
          ...
          db.query(
            `INSERT INTO quiz_attempts
               (student_id, course_id, course_level_id, level, total_questions,
                correct_answers, wrong_answers, score_percentage, passed)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [req.userId, courseId, courseLevelId, level,
             total, correctCount, wrong, percentage, passed ? 1 : 0],
            (err3, insertResult) => {
              if (err3) {
                console.error('[quiz/submit] insert attempt error:', err3);
                // Still return result even if save fails
                return res.status(200).json({
                  success: true, attempt_id: null,
                  total, correct: correctCount, wrong,
                  score_percentage: percentage, passed, details,
                  warning: 'Result calculated but could not be saved',
                });
              }

              const attemptId = insertResult.insertId;

              // ── Step 5: Save individual answers ──────────
              // Build rows: [attempt_id, question_id, selected_index, is_correct]
              ...
              db.query(
                ...
                [answerRows],
                (err4) => {
                  if (err4) console.warn('[quiz/submit] insert answers warning:', err4.message);

                  // ── Step 6: Update enrollment progress ───
                  if (passed) {
                    db.query(
                      `UPDATE enrollments
                       SET quiz_completed = 1,
                           progress = LEAST(1, progress + 0.2)
                       WHERE student_id = ? AND course_id = ?`,
                      [req.userId, courseId],
                      (err5) => {
                        if (err5) console.warn('[quiz/submit] progress update warning:', err5.message);
                      }
                    );
                  }
...
//
//  Returns past quiz attempts for the student.
//
//  Response:
//  {
//    "success": true,
//    "attempts": [
//      {
//        "id": 17, "level": "Beginner", "score_percentage": 80,
//        "correct_answers": 4, "wrong_answers": 1, "total_questions": 5,
//        "passed": true, "attempted_at": "2026-04-10T14:00:00Z"
//      },
//      ...
//    ]
//  }
...
router.get('/:courseId/quiz/history', auth, (req, res) => {
  ...
  db.query(
    `SELECT id, level, total_questions, correct_answers, wrong_answers,
            score_percentage, passed, attempted_at
     FROM quiz_attempts
     WHERE student_id = ? AND course_id = ?
