...
//  ✅ Cloudinary storage — ملفات دائمة لا تُمسح عند redeploy
// ============================================================
...
// ✅ Cloudinary يرجع URL كامل في file.path مباشرة
...
router.post('/', auth, (req, res) => {
  uploadCourseImage(req, res, (uploadErr) => {
    ...
    // ✅ Cloudinary يرجع URL كامل في req.file.path
...
router.get('/:id', auth, (req, res) => {
  ...
  db.query(courseSql, [req.params.id, req.userId], (err, courseRows) => {
    ...
    const quizSql = `
      SELECT
        qq.id, qq.course_level_id, qq.question_text,
  ...
  uploadCourseImage(req, res, (uploadErr) => {
    ...
    db.query(
      ...
      (err, rows) => {
        ...
        // ✅ لو رُفعت صورة جديدة نستخدم URL تاعها، وإلا نحتفظ بالقديمة
        ...
        db.query(
          ...
          (err2) => {
            if (err2) return res.status(500).json({ success: false, message: 'Database error' });
            res.status(200).json({
              success: true, message: 'Course updated', imagePath: newImagePath,
            });
          }
        );
      }
      ...
      (err, rows) => {
        ...
        const levelRows = levels.map(l => {
          ...
          return [
            ...
            lvl,
            l.video_url    || null,
            filePath(files, `video_file_${lvl}`),   // ✅ Cloudinary URL
            l.text_content || null,
            l.quiz_note    || null,
            filePath(files, `pdf_course_${lvl}`),    // ✅ Cloudinary URL
            filePath(files, `pdf_exercise_${lvl}`),  // ✅ Cloudinary URL
          ];
        });
