...
router.get('/:id/progress', auth, (req, res) => {
  ...
  db.query(
    `SELECT video_progress, video_position, pdf_opened, quiz_completed,
            pdf_page, pdf_exercise_page
    ...
    (err, rows) => {
      ...
      if (!rows.length) {
        return res.status(200).json({
          success: true,
          video_progress: 0, video_position: 0,
          pdf_opened: false, quiz_completed: false,
          pdf_page: 1, pdf_exercise_page: 1,
        });
      }
      const r = rows[0];
      return res.status(200).json({
...
//  Body: { video_progress?, video_position?, pdf_opened?,
//          quiz_completed?, pdf_page?, pdf_exercise_page? }
  ...
  db.query(
    ...
    (err, rows) => {
      ...
      const videoPosition   = req.body.video_position    !== undefined
        ? parseInt(req.body.video_position)
        : (current.video_position || 0);
      ...
      const pdfPage         = req.body.pdf_page          !== undefined
        ? parseInt(req.body.pdf_page)
        : (current.pdf_page || 1);
      const pdfExercisePage = req.body.pdf_exercise_page !== undefined
        ? parseInt(req.body.pdf_exercise_page)
        : (current.pdf_exercise_page || 1);
      ...
      db.query(
        ...
        [videoProgress, videoPosition, pdfOpened, quizCompleted,
         pdfPage, pdfExercisePage,
